  ...
  return (
    ...
    overflow-visible'>
      ...
      items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden'>
        ...
        <h1 className='test-3xl font-bold mb-2 text-[#ff4d2d]'>FoodEase</h1>
      ...
      <div className='flex items-center gap-4'>
        {showSearch?<RxCross1 className='text-[#ff4d2d] md:hidden' onClick={()=>setShowSearch(false)}/>
        : <IoMdSearch size={25} className='text-[#ff4d2d] md:hidden'
      ...
      text-white text-[18px] shadow-xl font-semibold cursor-pointer uppercase' 
      onClick={()=>setShowInfo(prev=>!prev)}>
      {userData?.fullName.slice(0,1)}
      </div>
      {showInfo && <div className='fixed top-[80px] right-[10px] md:right-[10%] lg:right-[25%] w-[180px] bg-white
      shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]'>
      <div className='text-[17px] font-semibold'>{userData.fullName}</div>
      <div className='md:hidden text-[#ff2d4d] font-semibold cursor-pointer'>My Orders</div>
