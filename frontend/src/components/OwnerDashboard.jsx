...
text-center'><FaUtensils className='text-[#ff4d2d] w-14 h-14 sm:w-20 sm:h-20 mb-4'/>Welcome to {myShopData.name}</h1>
...
hover:shadow-2xl transition-all duration-300 w-full max-w-3xl relative'>
  ...
  <img src={myShopData.image} alt={myShopData.image} className='w-full h-48 sm:h-64 object-cover' />
        ...
        border-gray-100 hover:shadow-xl transition-shadow duration-300'>
           <div className='flex flex-col items-center text-center'>
             <FaUtensils className='text-[#ff4d2d] w-16 h-16 sm:w-20 sm:h-20 mb-4'/>
              ...
              <p className='text-gray-600 mb-4 text-sm sm:text-base'>
              Share your delicious creations with our customers by adding them to the menu </p>
                <button className='bg-[#ff4d2d] text-white px-5 sm:px-6 py-2 rounded-full font-medium 
                shadow-md hover:bg-orange-600 transition-colors duration-200'
                onClick={()=>navigate("/add-item")}>Add Food</button>
           </div>
        </div>
      </div>}

{myShopData.items.length>0 && 
<div className='flex flex-col  hover:shadow-xl transition-shadow duration-300
